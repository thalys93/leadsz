...
export class MailService {
  ...
  private async getTransporter() {
    ...
    if (driver === 'console') {
      this.logger.log('Usando driver Console (sem envio, apenas imprime).');
      return nodemailer.createTransport({
  ...
  private renderWithLayout(
    ...
    const vars = { ...variables, appName: appConfig.name };
    const body = this.renderTemplate(contentTemplate, vars);
    return this.renderTemplate('layout.hbs', {
      appName: appConfig.name,
      pageTitle,
      iconUrl: appConfig.iconUrl,
      wordmarkUrl: appConfig.wordmarkUrl,
      body,
    });
  }
  ...
  async sendMail(to: string, subject: string, html: string) {
    ...
    const fromName = process.env.MAIL_FROM_NAME?.replace(/^["']|["']$/g, '');
    ...
    try {
      ...
      this.logger.error(`Falha ao enviar email (driver=${driver}): ${msg}`);
      const recoverable =
        code === 'EAUTH' ||
        code === 'ETIMEDOUT' ||
        code === 'ECONNRESET' ||
  ...
  async sendInvitationMail(to: string, companyName: string, acceptUrl: string) {
    const title = `Convite para ${companyName}`;
    const message = `Você foi convidado(a) para entrar em ${companyName} no ${appConfig.name}. Acesse: ${acceptUrl}`;
    await this.sendNotificationMail(to, title, message);
  }
}
