...
export class UserService {
  ...
  async create(createUserDto: CreateUserDto) {
    ...
    const user = this.usersRepository.create({
      ...
      jobTitle: createUserDto.jobTitle?.trim() || null,
      roles,
      settings: createUserDto.settings ?? {},
      companyId: createUserDto.companyId ?? null,
      companyRole: createUserDto.companyRole ?? null,
    });
  ...
  async update(id: string, updateUserDto: UpdateUserDto) {
    ...
    const trimToNullFields = new Set(['jobTitle', 'phone', 'website']);
    ...
    for (const field of allowedFields) {
      if (updateUserDto[field] !== undefined) {
        ...
        if (field === 'name' && typeof value === 'string') {
          filteredUpdateData[field] = value.trim();
        } else if (trimToNullFields.has(field) && typeof value === 'string') {
          filteredUpdateData[field] = value.trim() || null;
        } else {
